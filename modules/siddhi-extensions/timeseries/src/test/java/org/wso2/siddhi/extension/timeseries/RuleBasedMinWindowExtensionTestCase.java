...
public class RuleBasedMinWindowExtensionTestCase {
    static final Logger log = Logger.getLogger(RuleBasedMinWindowExtensionTestCase.class);
    ...
    @Test
    public void testRuleBasedMinWindowExtension() throws InterruptedException {
        log.info("RuleBasedMinWindowExtension TestCase");
        SiddhiManager siddhiManager = new SiddhiManager();

        String inStreamDefinition = "@config(async = 'true')define stream inputStream (price double);";
        String query = ("@info(name = 'query1') from inputStream#window.timeseries:ruleMin(price, 4, 3) " +
                "select *" +
                ...
                for (Event event : inEvents) {
                    ...
                    switch (count) {
                        case 1:
                            ...
                            break;
                        case 2:
                            Assert.assertEquals(59.81, event.getData(0));
                            break;
                        case 3:
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            Assert.assertEquals(56.35, event.getData(0));
                            break;
                        case 6:
                            Assert.assertEquals(57.42, event.getData(0));
                            break;
                        case 7:
                            Assert.assertEquals(45.74, event.getData(0));
                            break;
                        case 8:
                            Assert.assertEquals(46.5, event.getData(0));
                            break;
                        default:
