...
public class KalmanWindowExtensionTestCase {
    static final Logger log = Logger.getLogger(KalmanWindowExtensionTestCase.class);
    ...
    @Test
    public void testKalmanMaxWindowExtension() throws InterruptedException {
        log.info("KalmanMaxWindowExtension TestCase");
        SiddhiManager siddhiManager = new SiddhiManager();

        String inStreamDefinition = "@config(async = 'true')define stream inputStream (price double);";
        String query = ("@info(name = 'query1') from inputStream#window.timeseries:kalmanMax(price, 0.000001,0.0001, 25)  " +
                "select *" +
    ...
    @Test
    public void testKalmanMinWindowExtension() throws InterruptedException {
        log.info("KalmanMinWindowExtension TestCase");
        SiddhiManager siddhiManager = new SiddhiManager();

        String inStreamDefinition = "@config(async = 'true')define stream inputStream (price double);";
        String query = ("@info(name = 'query1') from inputStream#window.timeseries:kalmanMin(price, 0.000001,0.0001, 25) " +
                "select *" +
                "insert into outputStream;");
        ExecutionPlanRuntime executionPlanRuntime = siddhiManager.createExecutionPlanRuntime(inStreamDefinition + query);
