...
public class KernelExtensionTestCase {
    ...
    @Test
    public void testKernelMaxWindowExtension() throws InterruptedException {
        log.info("KernelExtensionTestCase TestCase");
        SiddhiManager siddhiManager = new SiddhiManager();

        String inStreamDefinition = "@config(async = 'true')define stream inputStream (price double);";
        String query = ("@info(name = 'query1') from inputStream#window.timeseries:kernelMax(price, 4, 17) " +
                "select *" +
    ...
    public void testKernelMinMaxStreamProcessorExtension() throws InterruptedException {
        log.info("KernelMinMaxStreamProcessorExtension TestCase");
        SiddhiManager siddhiManager = new SiddhiManager();

        String inStreamDefinition = "@config(async = 'true')define stream inputStream (price double);";
        String query = ("@info(name = 'query1') from inputStream#timeseries:kernelMinMax(price, 3, 16) " +
                "select *" +
                "insert into outputStream;");
        ExecutionPlanRuntime executionPlanRuntime = siddhiManager.createExecutionPlanRuntime(inStreamDefinition + query);
