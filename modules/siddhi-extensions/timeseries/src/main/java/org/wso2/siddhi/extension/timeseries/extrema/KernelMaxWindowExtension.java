...
public class KernelMaxWindowExtension extends WindowProcessor implements FindableProcessor {
    ...
    private ExtremaHelper extremaHelper;

    @Override
    protected void init(ExpressionExecutor[] attributeExpressionExecutors, ExecutionPlanContext executionPlanContext) {
        if (attributeExpressionExecutors.length != 3) {
            throw new ExecutionPlanValidationException("Invalid no of arguments passed to KernelMaximaWindow, required 3, but found " + attributeExpressionExecutors.length);
        }
        if (attributeExpressionExecutors[0].getReturnType() != Attribute.Type.DOUBLE) {
            throw new ExecutionPlanValidationException("Invalid parameter type found for the argument of KernelMaxWindow, " +
        ...
        extremaHelper = new ExtremaHelper();
    }
    ...
    protected void process(ComplexEventChunk<StreamEvent> streamEventChunk, Processor nextProcessor, StreamEventCloner streamEventCloner) {
        ...
        while (streamEventChunk.hasNext()) {
            ...
            if (eventStack.size() < windowSize) {
                ...
                Queue<Double> output = extremaHelper.smooth(priceStack, bw);
                ...
                Integer maxPos = extremaHelper.findMax(output, 1);
                if (maxPos != null) {
                    //values 5 and 3 are optimized values for stock market domain, these value may change for other domains
                    Integer maxPosEvnt = extremaHelper.findMax(priceStack, windowSize / 5, windowSize / 3);

                    if (maxPosEvnt != null && maxPosEvnt - maxPos <= windowSize / 5 && maxPos - maxPosEvnt <= windowSize / 2) {
                        StreamEvent minimumEvent = (StreamEvent) eventStack.toArray()[maxPosEvnt];
