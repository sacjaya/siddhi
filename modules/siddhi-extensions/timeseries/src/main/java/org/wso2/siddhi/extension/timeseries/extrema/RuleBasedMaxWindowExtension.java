...
public class RuleBasedMaxWindowExtension extends WindowProcessor implements FindableProcessor {
    ...
    protected void init(ExpressionExecutor[] attributeExpressionExecutors, ExecutionPlanContext executionPlanContext) {
        if (attributeExpressionExecutors.length != 3) {
            throw new ExecutionPlanValidationException("Invalid no of arguments passed to MaximaWindow, required 3, but found " + attributeExpressionExecutors.length);
        }
        if (attributeExpressionExecutors[0].getReturnType() != Attribute.Type.DOUBLE) {
            throw new ExecutionPlanValidationException("Invalid parameter type found for the argument of RuleBasedMaxWindow, " +
    ...
    protected void process(ComplexEventChunk<StreamEvent> streamEventChunk, Processor nextProcessor, StreamEventCloner streamEventCloner) {
        ...
        while (streamEventChunk.hasNext()) {
            ...
            if (eventStack.size() < (bwl + bwr)) {
                ...
                if (extremaHelper.max(priceStack) == priceStack.toArray()[bwl]) {
                    returnChunk.add((StreamEvent) eventStack.toArray()[bwl]);
                }
                eventStack.remove();
                priceStack.remove();
